import { constants, InteractionResponse } from './shared';
import type { Page } from 'playwright';

export function hookInteractionApi(page: Page): void {
    page.exposeFunction(
        constants.browserPageHook,
        async (path: string, ...args: any): Promise<InteractionResponse> => {
            ...
            while (refNames.length) {
                ...
                if (next === undefined && refNames) {
                    return {
                        type: `error`,
                        msg: `ERROR: calling browser API with unknown path: "${path}"`,
                    };
                }
                context = current;
                current = next;
